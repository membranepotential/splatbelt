...
export class PlyParser {
  ...
  decodeHeader(plyBuffer) {
    ...
    while (true) {
      ...
      const endHeaderTestChunk = new Uint8Array(
        plyBuffer,
        Math.max(0, headerOffset - readChunkSize * 2),
        readChunkSize * 2
      )
  ...
  parseToSplatBuffer(compressionLevel = 0, minimumAlpha = 1) {
    ...
    const { splatCount, propertyTypes, vertexData } = this.decodeHeader(
      this.plyBuffer
    )
    ...
    for (let row = 0; row < splatCount; row++) {
      ...
      if (alpha > minimumAlpha) {
        const newVertex = {}
        for (let propertyToRead of propertiesToRead)
          newVertex[propertyToRead] = rawVertex[propertyToRead]
      ...
      for (let i = 0; i < bucket.splats.length; i++) {
        ...
        if (compressionLevel === 0) {
          const center = new Float32Array(
            centerBuffer,
            outSplatIndex * bytesPerCenter,
            3
          )
          const scales = new Float32Array(
            scaleBuffer,
            outSplatIndex * bytesPerScale,
            3
          )
          const rot = new Float32Array(
            rotationBuffer,
            outSplatIndex * bytesPerRotation,
            4
          )
          ...
          if (propertyTypes['scale_0']) {
            ...
            scales.set([
              thf(Math.exp(rawVertex.scale_0)),
              thf(Math.exp(rawVertex.scale_1)),
              thf(Math.exp(rawVertex.scale_2)),
            ])
          } else {
          ...
          bucketCenterDelta
            .set(rawVertex.x, rawVertex.y, rawVertex.z)
            .sub(bucketCenter)
          bucketCenterDelta.x =
    ...
    const splatDataBufferSize =
      centerBuffer.byteLength +
      scaleBuffer.byteLength +
      colorBuffer.byteLength +
      rotationBuffer.byteLength
    ...
    new Uint8Array(unifiedBuffer, headerSize, centerBuffer.byteLength).set(
      new Uint8Array(centerBuffer)
    )
    new Uint8Array(
      unifiedBuffer,
      headerSize + centerBuffer.byteLength,
      scaleBuffer.byteLength
    ).set(new Uint8Array(scaleBuffer))
    new Uint8Array(
      unifiedBuffer,
      headerSize + centerBuffer.byteLength + scaleBuffer.byteLength,
      colorBuffer.byteLength
    ).set(new Uint8Array(colorBuffer))
    new Uint8Array(
      unifiedBuffer,
      headerSize +
        centerBuffer.byteLength +
        ...
        colorBuffer.byteLength,
      rotationBuffer.byteLength
    ).set(new Uint8Array(rotationBuffer))

    if (compressionLevel > 0) {
      const bucketArray = new Float32Array(
        unifiedBuffer,
        headerSize + splatDataBufferSize,
        buckets.length * 3
      )
