...
export class Viewer {
  constructor(options = {}) {
    ...
    if (options.halfPrecisionCovariancesOnGPU === undefined)
    ...
    this.initializeFromExternalUpdate =
      options.initializeFromExternalUpdate || false
    ...
    return new Promise((resolve, reject) => {
      ...
      const downloadProgress = (percent, percentLabel) => {
        ...
        if (options.onProgress)
          options.onProgress(percent, percentLabel, 'downloading')
      }
      this.loadFileToSplatBuffer(
        fileURL,
        ...
        .then((splatBuffer) => {
          ...
          this.loadSplatBuffersIntoMesh(
            [splatBuffer],
            [splatBufferOptions],
            options.showLoadingSpinner
          ).then(() => {
            if (options.onProgress)
              options.onProgress(100, '100%', 'processing')
            resolve()
          })
        })
        .catch((e) => {
          reject(
            new Error(`Viewer::loadFile -> Could not load file ${fileURL}`)
          )
        })
    })
  }
  ...
  loadFiles(files, showLoadingSpinner = true, onProgress = undefined) {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const downloadPromise = this.loadFileToSplatBuffer(
          files[i].path,
          meshOptionsForFile.splatAlphaRemovalThreshold,
      ...
      Promise.all(downLoadPromises)
        ...
        .catch((e) => {
          reject(
            new Error(`Viewer::loadFiles -> Could not load one or more files.`)
          )
        })
    })
  }

  loadFileToSplatBuffer(
    fileURL,
    plySplatAlphaRemovalThreshold = 1,
    ...
    return new Promise((resolve, reject) => {
      ...
      if (SplatLoader.isFileSplatFormat(fileName)) {
        ...
        fileLoadPromise = new PlyLoader().loadFromURL(
          ...
          downloadProgress,
          0,
          plySplatAlphaRemovalThreshold
        )
      } else {
        reject(
          new Error(
            `Viewer::loadFileToSplatBuffer -> File format not supported: ${fileURL}`
          )
        )
      }
      fileLoadPromise
        ...
        .catch(() => {
          reject(
            new Error(
              `Viewer::loadFileToSplatBuffer -> Could not load file ${fileURL}`
            )
          )
        })
    })
    ...
    return new Promise((resolve) => {
      ...
      sortWorker.onmessage = (e) => {
        if (e.data.sortDone) {
          ...
          this.sortWorkerPrecomputedDistances = new Int32Array(
    ...
    return function (gatherAllNodes) {
      ...
      for (let i = 0; i < nodeCount; i++) {
        ...
        if (
          ...
          (outOfFovX ||
            outOfFovY ||
            distanceToNode > MaximumDistanceToRender) &&
          distanceToNode > ns
        ) {
          continue
        }
    ...
    return function (timeDelta) {
      ...
      if (this.transitioningCameraTarget) {
        ...
        let newFocusMarkerOpacity = Math.min(
          currentFocusMarkerOpacity + fadeInSpeed * timeDelta,
          1.0
        )
        ...
        if (currentFocusMarkerOpacity > 0) {
          ...
          let newFocusMarkerOpacity = Math.max(
            currentFocusMarkerOpacity - fadeOutSpeed * timeDelta,
            0.0
          )
          this.sceneHelper.setFocusMarkerOpacity(newFocusMarkerOpacity)
  ...
  updateInfo = (function () {
    ...
    return function () {
      ...
      const cameraPosString = `[${cameraPos.x.toFixed(
        5
      )}, ${cameraPos.y.toFixed(5)}, ${cameraPos.z.toFixed(5)}]`
      this.infoPanelCells.cameraPosition.innerHTML = cameraPosString

      const cameraLookAt = this.controls.target
      const cameraLookAtString = `[${cameraLookAt.x.toFixed(
        5
      )}, ${cameraLookAt.y.toFixed(5)}, ${cameraLookAt.z.toFixed(5)}]`
      ...
      if (this.showMeshCursor) {
        ...
        const cursorPosString = `[${cursorPos.x.toFixed(
          5
        )}, ${cursorPos.y.toFixed(5)}, ${cursorPos.z.toFixed(5)}]`
      ...
      } splats out of ${splatCount} (${renderPct.toFixed(2)}%)`
  ...
  updateView = (function () {
    ...
    return function (force = false, gatherAllNodes = false) {
      ...
      if (!this.sortRunning) {
        ...
        if (
          this.gpuAcceleratedSort &&
          (queuedTiers.length <= 1 || queuedTiers.length % 2 === 0)
        ) {
          this.splatMesh.computeDistancesOnGPU(
            tempMatrix,
