...
export type FrameSelection = NumFrameSelection | ListFrameSelection | ImageFrameSelection;
...
export type SFMState = {
    state: 'fresh' | 'pairing' | 'extraction' | 'matching' | 'mapping' | 'training' | 'done';
    progress: number;
}
...
export function createModel(projectId: number, uploads: Upload[]): Model {
    const frames = Object.fromEntries(uploads.map(upload => [upload.name, defaultFrameSelection(upload)]));
...
export function toWorkerConfig(model: Model) {
    const config = {
        frames: {
            uploads: Object.entries(model.frames).map(([name, selection]) => ({ name, ...selection }))
        },
