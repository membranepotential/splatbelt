...
async function generatePresignedUrl(key: string, method: 'GET' | 'PUT' = 'PUT') {
...
async function generatePresignedPartUrls(key: string, uploadId: string, partCount: number) {
	const obj = { Key: key, Bucket: S3_BUCKET, Region: AWS_REGION, UploadId: uploadId }
...
async function initiateMultipartUpload(key: string): Promise<string> {
	return s3.send(
		new CreateMultipartUploadCommand({ Key: key, Bucket: S3_BUCKET }),
	).then(({ UploadId }) => UploadId as string);
}


async function completeMultipartUpload(key: string, uploadId: string, parts: CompletedPart[]) {
	return s3.send(
...
async function prepareMultiPartTransfer(key: string, size: number) {
	...
	const urls = await generatePresignedPartUrls(
		key,
		uploadId,
		partCount,
	);
...
export async function prepare({ key, size }: { key: string, size: number }) {
	const upload = size > MAX_PART_SIZE ?
...
export async function complete({ key, uploadId, etags }: { key: string, uploadId: string, etags: string[] }): Promise<Response> {
