<script lang="ts">
	...
	$: {
		model.frames = Object.fromEntries(
			frameSelections
				...
				.map((setting) => [setting.key, setting.selection || { type: 'num', num: 10 }])
		);
	...
	function handlePairingChange(event: Event) {
		...
		if (value === 'exhaustive') {
			...
			model.pairing = { type: 'complex', sequential: 5, retrieval: 5, covisible: null };
		}
	}
...
<div class="container p-4">
	...
	<div class="mb-4">
		...
		<div class="flex items-center mb-2">
			<select value={model.pairing.type} on:change={handlePairingChange} class="border p-2">
